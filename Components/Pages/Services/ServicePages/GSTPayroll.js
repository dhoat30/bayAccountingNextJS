...
function GSTPayroll({ heroImageData, servicesData, technologyPartnersData }) {
    ...
    // list items cards 
    const helpCards = singleServiceData[0].acf.flexible_content[1].list_items.map(item => {
        return (
            <ListItems listContent={item.list_item} />
        )
    })

    const gstCards = singleServiceData[0].acf.flexible_content[3].list_items.map(item => {
        return (
            <ListItems listContent={item.list_item} />
        )
    })
